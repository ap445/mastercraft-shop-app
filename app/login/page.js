...
export default function LoginPage(){
  ...
  async function submit(e){ e.preventDefault(); setBusy(true); setError('');
    try{ const res=await fetch('/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({employeeCode,pin})}); const data=await res.json(); if(!res.ok) throw new Error(data.error||'Login failed'); router.push(data.user.role==='employee'?'/employee':data.user.role==='admin'?'/admin':'/supervisor'); router.refresh(); }
