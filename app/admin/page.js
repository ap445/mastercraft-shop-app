...
function renderCalWeek(week,ops,maxTracks,showDayName){const today=ymdKey(new Date());const {placed,hiddenByDay}=weekLayout(week,ops,maxTracks);const rows=Math.max(1,placed.reduce((m,p)=>Math.max(m,p.track+1),0));return <div className="cal-week" key={ymdKey(week[0])}>
  ...
  return <main className="shell"><div className="topbar"><a className="brand" href="/admin"><BrandMark />MASTERCRAFT ADMIN</a><div className="navlinks"><a className="navlink" href="#guide">Guide</a><a className="navlink" href="#records">Records</a><a className="navlink" href="/admin/job">Jobs</a><a className="navlink" href="#shop-calendar">Calendar</a><a className="navlink" href="#materials">Materials</a><a className="navlink" href="/supervisor">Supervisor Board</a><a className="navlink" href="/admin/costing">Job Costing</a><a className="navlink" href="/admin/payroll">Payroll</a><button className="toplink" onClick={logout}>Sign out</button></div></div><div className="container">
    ...
    <div className="grid two">
      ...
      <div id="departments" className="card">
        <button type="button" className="collapse-toggle" onClick={()=>toggleForm('departments')}><span className="collapse-caret">{openForms.departments?'▾':'▸'}</span><div><div className="kicker">Organization</div><h2>Departments</h2></div></button>
        {openForms.departments?<form onSubmit={e=>save('department',e)}><p className="muted">Create these first so employees and operations have a home.</p><div className="field"><label>Department name</label><input value={forms.department.name} onChange={e=>change('department','name',e.target.value)} placeholder="Welding" /></div><button disabled={busy} className="btn secondary">ADD DEPARTMENT</button><div className="simple-list">{data.departments.map(d=><span className="badge" key={d.id}>{d.name}</span>)}</div></form>:<div className="simple-list">{data.departments.map(d=><span className="badge" key={d.id}>{d.name}</span>)}</div>}
      </div>
    ...
    <section id="records"><div className="section-heading"><div><div className="kicker">Reference library</div><h2>View shop records</h2><p className="muted">Look up existing jobs, materials, expectations, and people. Click a row to see everything linked to it, or edit it directly from there.</p></div></div>
      ...
      <div className="card">
        {recordsTab==='jobs'?<div className="table-wrap"><table className="table compact-table"><thead><tr><th>Job</th><th>Customer</th><th>Status</th><th></th></tr></thead><tbody>{data.jobs.length===0?<tr><td colSpan="4" className="muted">No jobs have been created yet.</td></tr>:data.jobs.map(j=><tr key={j.id} style={{cursor:'pointer',background:recordDetail?.type==='job'&&String(recordDetail.id)===String(j.id)?'#f3f4f6':undefined}} onClick={()=>setRecordDetail({type:'job',id:j.id})}><td><strong>{j.job_number}</strong><br/><span className="muted">{j.description}</span></td><td>{j.customer_name||'—'}</td><td><span className="badge">{j.status.replaceAll('_',' ')}</span></td><td><a className="xref" href={`/admin/job?job=${j.id}`} onClick={e=>e.stopPropagation()}>Manage</a></td></tr>)}</tbody></table></div>:null}
        {recordsTab==='scheduling'?<div className="table-wrap"><table className="table compact-table"><thead><tr><th>Job</th><th>Operation</th><th>Dept</th><th>Est.</th><th>Planned</th><th>Status</th></tr></thead><tbody>{data.operations.length===0?<tr><td colSpan="6" className="muted">No operations have been scheduled yet.</td></tr>:data.operations.map(o=><tr key={o.id} style={{cursor:'pointer',background:recordDetail?.type==='operation'&&String(recordDetail.id)===String(o.id)?'#f3f4f6':undefined}} onClick={()=>setRecordDetail({type:'operation',id:o.id})}><td><strong>{o.job_number}</strong></td><td>#{o.sequence_no} {o.operation_name}</td><td>{o.department_name}</td><td>{o.estimated_hours||'—'}</td><td>{o.planned_start?String(o.planned_start).slice(0,10):'—'}</td><td><span className={'badge '+(o.status==='in_progress'?'active':'')}>{String(o.status).replaceAll('_',' ')}</span></td></tr>)}</tbody></table></div>:null}
        {recordsTab==='team'?<div className="table-wrap"><table className="table compact-table"><thead><tr><th>Code</th><th>Name</th><th>Department</th><th>Role</th></tr></thead><tbody>{data.employees.map(e=><tr key={e.id} style={{cursor:'pointer',background:recordDetail?.type==='employee'&&String(recordDetail.id)===String(e.id)?'#f3f4f6':undefined}} onClick={()=>setRecordDetail({type:'employee',id:e.id})}><td>{e.employee_code}</td><td>{e.full_name}</td><td>{e.department_name||'—'}</td><td><span className="badge">{e.role}</span>{e.active===false?<span className="badge"> inactive</span>:null}</td></tr>)}</tbody></table></div>:null}
        {recordsTab==='materials'?<div className="table-wrap"><table className="table compact-table"><thead><tr><th>Item code</th><th>Description</th><th>UOM</th><th>Cost</th><th></th></tr></thead><tbody>{data.materials.length===0?<tr><td colSpan="4" className="muted">No materials have been added yet.</td></tr>:data.materials.map(m=><tr key={m.id} style={{cursor:'pointer',background:recordDetail?.type==='material'&&String(recordDetail.id)===String(m.id)?'#f3f4f6':undefined}} onClick={()=>setRecordDetail({type:'material',id:m.id})}><td><strong>{m.item_code}</strong></td><td>{m.description}</td><td>{m.unit_of_measure}</td><td>{m.standard_cost==null?'—':`$${Number(m.standard_cost).toFixed(2)}`}</td><td>{m.active===false?<span className="badge">inactive</span>:null}</td></tr>)}</tbody></table></div>:null}
