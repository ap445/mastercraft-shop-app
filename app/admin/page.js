...
function renderCalWeek(week,ops,maxTracks,showDayName){const today=ymdKey(new Date());const {placed,hiddenByDay}=weekLayout(week,ops,maxTracks);const rows=Math.max(1,placed.reduce((m,p)=>Math.max(m,p.track+1),0));return <div className="cal-week" key={ymdKey(week[0])}>
  ...
  return <main className="shell"><div className="topbar"><a className="brand" href="/admin">MASTERCRAFT ADMIN</a><div className="navlinks"><a className="navlink" href="#guide">Guide</a><a className="navlink" href="#records">Records</a><a className="navlink" href="/admin/job">Jobs</a><a className="navlink" href="#shop-calendar">Calendar</a><a className="navlink" href="#materials">Materials</a><a className="navlink" href="/supervisor">Supervisor Board</a><a className="navlink" href="/admin/costing">Job Costing</a><a className="navlink" href="/admin/payroll">Payroll</a><button className="toplink" onClick={logout}>Sign out</button></div></div><div className="container">
