...
export async function POST(request) {
  try {
    ...
    const openR = await query('select job_id from time_entries where employee_id=$1 and stopped_at is null limit 1', [auth.session.employeeId]);
    ...
    const r = await query(
      `insert into material_transactions(job_id,material_id,custom_material_name,employee_id,transaction_type,quantity,unit_cost,notes)
       values($1,$2,$3,$4,$5,$6,$7,$8) returning id,occurred_at`,
      [open.job_id || null, materialId || null, materialId ? null : customName, auth.session.employeeId, transactionType, qty, unitCost, notes]
    );
